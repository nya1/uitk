...
import { Editable, useEditableItem } from "./useEditableItem";
...
interface tabstripHookProps extends Partial<Editable> {
...
export const useTabstrip = ({
  ...
  const handleDrop = useCallback(
    (fromIndex: number, toIndex: number) => {
      ...
      if (toIndex === -1) {
        console.log(`select last item `, { data: collectionHook.data });
      } else {
  ...
  const handleClick = useCallback(
    (evt: MouseEvent<Element>, tabIndex: number) => {
      ...
      if (!dragDropHook.isDragging) {
        console.log("useTabstrip handleClick");
  ...
  const addTab = useCallback(
    ...
    (indexPosition: number = collectionHook.data.length - 1) => {
      console.log(`addTab @ ${indexPosition}`);
      const tabId =
      ...
      collectionHook.dispatch({
        ...
        idRoot,
        element: (
          <Tab data-editable={true} label={`New Tab ${count}`} id={tabId} />
        ),
