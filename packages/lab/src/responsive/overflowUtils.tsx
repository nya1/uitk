...
export const isCollapsed = (item: OverflowItem) => item.collapsed === true;
export const isCollapsing = (item: OverflowItem) => item.collapsing === true;
...
export const popNextItemByPriority = (items: OverflowItem[]) => {
...
export const measureOverflowItems = (
  ...
  const measurements = items.map(({ id }) => {
    ...
    if (childElement) {
      return childElement ? measureElementSize(childElement, dimension) : 0;
    } else {
      return 0;
      // throwing fails some tests in CI only, need to investigate why
      // throw Error(
      //   `overflowUtils.measureOverflowItems no element found for id ${id}`
      // );
    }
  });
